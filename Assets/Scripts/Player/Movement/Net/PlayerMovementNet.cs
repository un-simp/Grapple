...
namespace Wildflare.Player.Movement.Net
{
    [RequireComponent(typeof(PlayerInputNet))]
    public class PlayerMovementNet : NetworkBehaviour
    {
        //Assignables
        public Transform orientation;
        [SerializeField] private Transform groundCheck;
        [HideInInspector] public PlayerInputNet input;
        public Camera cam;
        ...
        //Layers
        [Tooltip("Player Will Slide Down When Contacting Layer")] [SerializeField]
        private LayerMask slideMask;
        ...
        private void Awake()
        {
            ...
            input = GetComponent<PlayerInputNet>();
            DOTween.Init();
        ...
        private void Movement()
        {
            ...
            var dir = (orientation.forward * input.yInput * verticalMultiplier +
                       orientation.right * input.xInput * horizontalMultiplier).normalized;
            rb.AddForce(dir * speed);
        }

        private void CounterMovement()
        {
            if (rb.velocity.magnitude > 0 && isGrounded) rb.AddForce(-rb.velocity.normalized * drag * speed);

            if (rb.velocity.magnitude < 0.5f && !isMoving && isGrounded) rb.velocity = Vector3.zero;
        ...
        {
            var tempVector = new Vector3(rb.velocity.x, 0, rb.velocity.z);
        ...
        private void Jump()
        {
            if (isGrounded)
            {
                ...
                if (currentVelocity > 6) rb.AddForce(orientation.forward * (jumpForce / 2));
        ...
        {
            if (col.gameObject.layer == 6) isGrounded = true;
        }

        private void OnCollisionExit(Collision col)
        {
            if (col.gameObject.layer == 6) isGrounded = false;
        }
